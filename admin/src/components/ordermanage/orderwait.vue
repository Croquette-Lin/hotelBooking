  ...
  <div>
      ...
      >
        ...
        <el-table-column label="操作" width="200" fixed="right" align="center">
          <template slot-scope="scope">
            <el-button size="mini" type="danger" @click="checkin(scope.$index, scope.row)">入住</el-button>
            <el-button size="mini" @click="cancel(scope.$index, scope.row)">取消订单</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
    ...
    <el-dialog title="提示" :visible.sync="delDialogVisible" width="30%">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="doIn">确定</el-button>
      </span>
    </el-dialog>

    <el-dialog title="提示" :visible.sync="delDialogVisible2" width="30%">
      <span>确定订单入住时间已逾期，需要取消订单？</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="delDialogVisible2 = false">取消</el-button>
        <el-button type="primary" @click="doCancel">确定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    _getlist() {
      this.$http.get("/api/order/getwaitList").then(response => {
    ...
    doIn() {
      ...
      this.$http
        .post(
          "/api/order/editRoomState",
          {
            r_id: this.orderDetail.r_id,
            number: this.orderDetail.h_number
          },
          {
            emulateJSON: true
          }
